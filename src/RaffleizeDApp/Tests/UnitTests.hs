...
createRaffleTests =
  ...
  where
    createNewRaffleTC1 TestInfo {..} = void $ do
        ddlTime <- slotToEndTime (unsafeSlotFromInteger 100)
        rddlTime <- slotToEndTime (unsafeSlotFromInteger 200)
        let cddl = timeToPlutus ddlTime
        let rddl = timeToPlutus rddlTime
        let config =
              RaffleConfig
                { rCommitDDL = cddl
                , rRevealDDL = rddl
